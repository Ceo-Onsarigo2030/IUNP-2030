...
const EMPTY_NOMINEE = { name: "", bio: "", photo_url: "", media_url: "" };
...
function NomineesPanel({ category, onClose }: { category: any; onClose: () => void }) {
  ...
  async function addNominee(e: React.FormEvent) {
    ...
    setSaving(true);
    const supabase = createClient();
    await supabase.from("gala_nominees").insert({
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-cream rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-6">
          <form onSubmit={addNominee} className="card-elegant p-4 space-y-2">
            <input required placeholder="Nominee full name" value={form.name} onChange={(e) => setForm((f: any) => ({ ...f, name: e.target.value }))}
              className="w-full rounded-lg border border-black/10 px-3 py-2 text-sm" />
            <textarea rows={2} placeholder="Short one-line bio (shown on the voting card)" value={form.bio} onChange={(e) => setForm((f: any) => ({ ...f, bio: e.target.value }))}
              className="w-full rounded-lg border border-black/10 px-3 py-2 text-sm resize-none" />
            <input placeholder="Profile photo URL" value={form.photo_url} onChange={(e) => setForm((f: any) => ({ ...f, photo_url: e.target.value }))}
              className="w-full rounded-lg border border-black/10 px-3 py-2 text-sm" />
            <input placeholder="Media link — photo/video of their work (optional)" value={form.media_url} onChange={(e) => setForm((f: any) => ({ ...f, media_url: e.target.value }))}
              className="w-full rounded-lg border border-black/10 px-3 py-2 text-sm" />
            <button type="submit" disabled={saving} className="btn-gold w-full !py-2.5 disabled:opacity-60">
              {saving ? <Loader2 className="size-4 animate-spin" /> : <><Plus className="size-4" /> Add nominee</>}
            </button>
          </form>
